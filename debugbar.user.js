...
//
(function() {
if (window && window.requestAnimationFrame && "performance" in window && window.performance.now) {
    window.BOOMR = window.BOOMR || {};
    window.BOOMR.fpsLog = [];

    function frame() {
        if (!BOOMR.version && window.BOOMR.fpsLog) {
            window.BOOMR.fpsLog.push(Math.round(window.performance.now()));
...
function initEmbeddedBoomerang() {
    ...
    */
    ...
    // Init
    //
    BOOMR.plugins.Continuity.init({ Continuity: { waitAfterOnload: window.CONTINUITY_WAIT_AFTER_ONLOAD }});

    if (document.readyState === "complete") {
        BOOMR.fireEvent("page_ready");
    } else {
        window.addEventListener("load", function() {
            BOOMR.fireEvent("page_ready");
        });
