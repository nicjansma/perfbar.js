...
function initEmbeddedBoomerang() {
    ...
    initEmbeddedBoomerangPlugins();

    BOOMR.init();
}

function initEmbeddedBoomerangPlugins() {
    //
    // Add the Boomerang ServerTiming, ResourceTiming and Continuity plugins
    //
(function() {
    BOOMR = UW.BOOMR || {};
    ...
    BOOMR.plugins.ServerTiming = {
      ...
      is_complete: function() {
        if (!impl.calculated) {
          ...
          let edgeTime = 0, total = 0, offload = {
              bytes: 0,
              count: 0
          }
          ...
          BOOMR.plugins.ServerTiming.edge = edgeTime
          BOOMR.plugins.ServerTiming.offload_bytes = offload.bytes
          BOOMR.plugins.ServerTiming.offload_percent = 100 * (offload.count / total)
          //console.info('BOOMR.plugins.ServerTiming', BOOMR.plugins.ServerTiming, offload.count, total)
        }

        if (!BOOMR.utils.serializeForUrl) {
          BOOMR.utils.serializeForUrl = function(value) {
